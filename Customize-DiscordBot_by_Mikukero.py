...
BOT_ID = 你的Discord机器人的ID
QWEN_API_KEY = "你的千问API Key"
TAVILY_API_KEY = "你的Tavily的Key"
DEV_URL = "https://discord.com/users/这里填你的Discord用户ID"
PUBLIC_ROLE_ID = 0
INNER_PORT = 5000
MIYUSHE_COOKIE = ""
HOYOLAB_COOKIE = ""
...
bot_status_data = {"online": True, "run_msg": "你先填什么启动文案就填什么"}

USER_DB = {
    "你可以设置管理员账号": {"pwd": "管理员账号的密码", "role": "admin", "email": "admin@你想填的.dev", "reg_time": time.time()}
}
EMAIL_CODE_CACHE = {}
BLACKLIST_UID = set()
USER_ONLINE = {}
...
def print_flash_logo():
    ...
    print(f"{GRADIENT_GREEN[1]}Firefly ready__火萤Ⅳ等待指示_byMikukero{COLOR_END}\n\n")
...
logger = logging.getLogger("自定义Bot")
...
# ==================== 【修复+你想填的主题】Flask 后台 ====================
web = Flask(__name__)
web.secret_key = "你想填的4_2026_Super_Secure_Key"
...
<div class="login-box">
    <h2 class="title">表情符号或emoji 你想填的 管理登录</h2>
    <form method="post">
...
<head>
<meta charset="utf-8">
<title>你想填的 - 管理后台</title>
<style>
*{box-sizing:border-box;margin:0;padding:0}
body {
    background: url('你的背景图来源网址.jpg/png') no-repeat center center fixed;
    background-size: cover;
    font-family: Arial, sans-serif;
    color: #fff;
}
...
"""
        ...
        text = """
🥰🥳🥳 你想填的（可填英文名或删掉这段） Full Functions🦽
/help Show help
...
async def keep_alive_task():
    while True:
        if bot_status_data["online"]:
            try:
                await bot.change_presence(activity=discord.Game("status"))#状态，自定义
            except:
                pass
